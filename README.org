...
| =denote-solo-modeline-function=    | =denote-solo--default-modeline-text=     | Function that formats the mode line text.    |

All options are available through =M-x customize-group RET denote-solo=.

** Customising the mode line format

By default the active solo is shown as ={denote: Studies}=.  To change
the format, set =denote-solo-modeline-function= to any function that
accepts a solo name string and returns a string:
...
#+begin_src emacs-lisp
(defun my/denote-solo-modeline (name)
  (concat "[ " name " ]"))

(setq denote-solo-modeline-function #'my/denote-solo-modeline)
#+end_src
